      ...
      <div class="mb-8">
        <div class="flex items-start justify-between">
          ...
          <div v-if="list.is_owner" class="flex items-center space-x-2">
            <button @click="showEditModal = true" class="btn btn-secondary flex items-center">
              <svg class="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
              </svg>
              <span class="hidden sm:inline">编辑</span>
            </button>
            <button @click="confirmDelete" class="btn btn-secondary text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center">
              <svg class="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showEditModal = false">
      <div class="bg-white dark:bg-dark-200 rounded-2xl p-6 w-full max-w-md mx-4">
        ...
        <div class="flex justify-end space-x-3 mt-6">
          ...
          <button @click="updateList" class="btn btn-primary">保存</button>
        </div>
      </div>
    </div>
  </div>
