        ...
        <div class="flex-1">
          ...
          <div class="flex flex-wrap items-center gap-3 mb-8">
            ...
            >
              ...
              {{ userRating?.rated ? (userRating.isAggregated ? `单集均分 ${userRating.rating}` : `我的评分 ${userRating.rating}`) : '评分' }}
            </button>
          </div>
...
const checkUserRating = async () => {
  ...
  try {
    ...
    if (result.rated) {
      ...
      // 如果是剧集且没有整部评分，检查是否有任何单集评分
      // 这样用户可以看到他们已经对该剧有评分记录
      try {
        const episodeRatingsResult = await ratingsApi.getRatings(appStore.currentEmbyUser.Id, {
          media_type: 'episode',
          limit: 100,
        })

        // 检查是否有该剧的单集评分（通过标题匹配）
        const showName = item.value.name
        const hasEpisodeRating = episodeRatingsResult.ratings?.some(r =>
          r.title && r.title.startsWith(showName + ' - ')
        )

        if (hasEpisodeRating) {
          // 找出该剧的所有单集评分，计算平均分
          const showEpisodeRatings = episodeRatingsResult.ratings.filter(r =>
            r.title && r.title.startsWith(showName + ' - ')
          )
          const avgRating = Math.round(
            showEpisodeRatings.reduce((sum, r) => sum + r.rating, 0) / showEpisodeRatings.length
          )

          userRating.value = {
            rated: true,
            rating: avgRating,
            review: `已评分 ${showEpisodeRatings.length} 集`,
            isAggregated: true,  // 标记这是聚合评分
          }
          ratingValue.value = avgRating
        }
      } catch (e) {
        // 忽略单集评分检查的错误
        console.log('No episode ratings found')
      }
    }
  } catch (e) {
